...
//	            translated = CatalogueApiUtils.getLFN(UsersHelper.getHomeDir(JAliEnCOMMander.user.getName())).getParentDir()
//	                                                                  .getCanonicalName();
...
//                + translated;
//	        }
		translated = FileSystemUtils.getAbsolutePath(JAliEnCOMMander.user.getName(), JAliEnCOMMander.curDir.getCanonicalName(), translated);
		LFN lfn = CatalogueApiUtils.getLFN(translated);
	        ...
	        if (translated.endsWith(separator)) {
	            ...
	            dir = CatalogueApiUtils.getLFN(translated.substring(0,translated.lastIndexOf('/')));
	        }

	        final LFN[] entries = (dir == null) ? new LFN[0] : directorylistLFNs(dir);

	        try {
	        	System.out.println("b,t,e,c:" +buffer + "," + translated + "," + entries +","+candidates);
	     ...
	     *  @param        entries                the list of LFNs to match
	     *  @param        candidates        the list of candidates to populate
	     *
	     *  @return  the offset of the match
	     */
	    @SuppressWarnings({ "unchecked", "rawtypes" })
		public int matchLFNs(String buffer, int cursor, String translated, LFN[] entries,
										List candidates) {
		...
		private LFN[] directorylistLFNs(LFN lfn){
			...
			try {
				ls = (JAliEnCommandls) JAliEnCOMMander.getCommand("ls",new Object[]{args});
			} catch (Exception e) {
			}
			ls.silent();
			ls.execute();
