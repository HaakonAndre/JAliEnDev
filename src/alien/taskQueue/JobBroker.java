...
public class JobBroker {
	...
	public static HashMap<String, Object> getMatchJob(final HashMap<String, Object> matchRequest) {
		try (DBFunctions db = TaskQueueUtils.getQueueDB()) {
			...
			HashMap<String, Object> matchAnswer = new HashMap<String, Object> ();
			HashMap<String, Object> waiting = new HashMap<String, Object>(); 
			
			// Checking if the CE is open
			int openQueue = checkQueueOpen((String) matchRequest.get("CE"));
			if ( openQueue != 1 ) {
			...
			if(waiting.containsKey("entryId")){
				...
				String installedPackages = (String) matchRequest.get("InstalledPackages");
				...
				if(waiting.containsKey("packages")){
					String packages = (String) waiting.get("packages");
					logger.log(Level.INFO, "Telling the site to install packages '"+packages+"'");

					ArrayList<String> list = new ArrayList<String>(Arrays.asList( packages.split(",") ));
			...
			if (matchAnswer.containsKey("queueId")){
				// success!!
				matchAnswer.put("Code", 1);
				
				// TODO:joblog
				// putlog($queueid, "state", "Job state transition from WAITING to ASSIGNED (to $queueName)");
				
				JobToken jobToken = TaskQueueUtils.insertJobToken((int) matchAnswer.get("queueId"), 
																  (String) matchAnswer.get("User"), false);
				
				
				if(jobToken==null || !jobToken.spawnToken(db) ){
					logger.log(Level.INFO, "The job already had a jobToken (or failed creating)!");
					db.setReadOnly(true);
					if( db.query("select * from QUEUE where queueId=?",false,(int) matchAnswer.get("queueId")) ){
						Job j = new Job(db,false);
	...
	private static HashMap<String, Object> getWaitingJobForAgentId(HashMap<String, Object> waiting) {
		try (DBFunctions db = TaskQueueUtils.getQueueDB()) {
			if (db == null)
				return null;
			
			String agentId = (String) waiting.get("entryId");
			String host = (String) waiting.get("Host");
			String ceName = (String) waiting.get("CE");
			...
			String extra="";
			if( (int)waiting.get("Remote")==1 ){
			    extra = "and timestampdiff(SECOND,mtime,now())>=ifnull(remoteTimeout,$self->{DEFAULTREMOTETIMEOUT})";
			}

			db.query("UPDATE QUEUE set statusId="+JobStatus.ASSIGNED.getAliEnLevel()+" ,siteid=?, exechostid=?, sent=now() "+ 
					  "where statusId="+JobStatus.WAITING.getAliEnLevel()+" and agentid=? "+extra+" and @assigned_job:=queueid limit 1",
					  false, siteId, hostId, agentId);
			...
			if(db.getUpdateCount()>0){
				...
				int queueId;
			  	...
			  	db.query("select queueId, origjdl jdl, user from QUEUEJDL join QUEUE using (queueid) "
			  			+ "join QUEUE_USER using (userId) where queueId=@assigned_job", false);
			  	...
			  	db.query("update SITEQUEUES set ASSIGNED=ASSIGNED+1 where siteid=?",false,siteId);
			  	db.query("update SITEQUEUES set WAITING=WAITING-1 where siteid=?",false,siteId);
			  	
			  	HashMap<String, Object> job = new HashMap<String, Object>();
			  	job.put("queueId", queueId);
			  	job.put("JDL", jdl);
			  	job.put("User", user);
	...
	private static int checkQueueOpen(String ce) {
		try (DBFunctions db = TaskQueueUtils.getQueueDB()) {
			if (db == null)
				return -2;
			...
			db.setReadOnly(true);
		  	...
		  	if (db.moveNext()){
		  		if( db.geti(1)>0 ){
		  			...
		  			// TaskQueueUtils.setSiteQueueStatus(ce, "closed-blocked");
		  		}
		  	}
	...
	public static HashMap<String, Object> getNumberWaitingForSite(final HashMap<String, Object> matchRequest) {
		try (DBFunctions db = TaskQueueUtils.getQueueDB()) {
			...
			HashMap<String, Object> matchAnswer = new HashMap<String, Object> ();
			matchAnswer.put("Code", 0);
			
			String where = "";
			String ret = "sum(counter) as counter";
			if (matchRequest.containsKey("Return")) ret = (String) matchRequest.get("Return");
			...
			if (matchRequest.containsKey("Users")) {
				ArrayList<String> users = (ArrayList<String>) matchRequest.get("Users");
				String orconcat = " and (";
			...
			if( (Integer) matchRequest.get("Remote")==1 ){
				...
				// $self->{CONFIG}->{CACHE_SERVICE_ADDRESS}?ns=jobbroker&key=remoteagents
			  	...
			  	db.query("select distinct agentId from QUEUE where agentId is not null and statusId=5 "
			  	...
			  	if (db.moveNext()){
				  	...
				  	do{
				  		final Integer agentid = db.geti("agentId");
				  		where += agentid+",";
				  	} while (db.moveNext());
			...
			String q = "select "+ret+" from JOBAGENT where 1=1 "+where+
