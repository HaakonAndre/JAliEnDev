	...
	public String transfer(final PFN source, final PFN target) throws IOException {
		...
		try{
	        ...
	        if (exitStatus.getExtProcExitStatus() != 0){
				...
				if (sMessage.indexOf("unable to connect to destination")>=0 || sMessage.indexOf("No servers are available to write the file.")>=0 || sMessage.indexOf("Unable to create")>=0 || sMessage.indexOf("dest-size=0 (source or destination has 0 size!)")>=0)
