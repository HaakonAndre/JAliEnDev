...
public class Base64Moded {
	 ...
	 *            Specified options
	 * @return The Base64-encoded object
	 * @see Base64Moded#GZIP
	 * @see Base64Moded#DONT_BREAK_LINES
	 * @since 2.0
	 */
	...
	public static String encodeBytes(final byte[] source, final int off, final int len, final int options) {
		...
		if (gzip == GZIP)
			try (java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream();
					Base64Moded.OutputStream b64os = new Base64Moded.OutputStream(baos, ENCODE | dontBreakLines);
	...
	public static byte[] decode(final byte[] source, final int off, final int len) {
		...
		for (i = off; i < off + len; i++) {
			...
			if (sbiDecode >= WHITE_SPACE_ENC) // White space, Equals sign or better
			{
				...
				System.out.println("Bad Base64 input character at " + i + ": " + source[i] + "(decimal)");
				return null;
			} // end else:
	...
	public static boolean encodeToFile(final byte[] dataToEncode, final String filename) {
		...
		try (Base64Moded.OutputStream bos = new Base64Moded.OutputStream(new java.io.FileOutputStream(filename), Base64Moded.ENCODE)) {
	...
	public static boolean decodeToFile(final String dataToDecode, final String filename) {
		boolean success = false;
		try (Base64Moded.OutputStream bos = new Base64Moded.OutputStream(new java.io.FileOutputStream(filename), Base64Moded.DECODE)) {
	...
	public static byte[] decodeFromFile(final String filename) {
		...
		try {
			...
			if (file.length() > Integer.MAX_VALUE) {
				System.out.println("File is too big for this convenience method (" + file.length() + " bytes).");
				return null;
			} // end if: file too big for int index
			buffer = new byte[(int) file.length()];

			// Open a stream
			try (Base64Moded.InputStream bis = new Base64Moded.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(file)), Base64Moded.DECODE)) {
				// Read until done
	...
	public static String encodeFromFile(final String filename) {
		...
		try {
			...
			// Open a stream
			try (Base64Moded.InputStream bis = new Base64Moded.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(file)), Base64Moded.ENCODE)) {
				// Read until done
				while ((numBytes = bis.read(buffer, length, 4096)) >= 0)
					length += numBytes;
			}
			...
			// Save in a variable to return
			encodedData = new String(buffer, 0, length, Base64Moded.PREFERRED_ENCODING);
	...
	public static class InputStream extends java.io.FilterInputStream {
		 ...
		 * @param options
		 *            Specified options
		 * @see Base64Moded#ENCODE
		 * @see Base64Moded#DECODE
		 * @see Base64Moded#DONT_BREAK_LINES
		 * @since 2.0
		 */
	...
	public static class OutputStream extends java.io.FilterOutputStream {
		...
		public void write(final int theByte) throws java.io.IOException {
			...
			if (DECODABET[theByte & 0x7f] > WHITE_SPACE_ENC) {
				...
				{
					final int len = Base64Moded.decode4to3(buffer, 0, b4, 0);
					out.write(b4, 0, len);
